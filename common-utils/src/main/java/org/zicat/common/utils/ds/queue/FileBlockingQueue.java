...
public class FileBlockingQueue<E> extends AbstractQueue<E> implements Closeable, BlockingQueue<E> {
	...
	private void initActiveSegments() throws IOException {
		...
		for (int i = 0; i < size; i++) {
			...
			if (readLength != length)
				throw new IOException("read bytes error, want to read " + length + " bytes, but really read "
						+ readLength + " bytes, file name is " + segmentFactory.idFile.getPath());
	...
	/**
	 * read data blocking timeout < 0 then blocking until readable
	 * 
	 * @param timeout
	 * @param unit
	 * @return
	...
	private boolean add0(E element) throws IOException {
		...
		lock.lock();
		try {
			// if return false, another thread have been created the new segment and that
			// thread will be signal.
