	...
	private void start0(final ConsumerHandler<E> consumerHandler) throws InterruptedException {
		...
		try {
			...
			while (true) {
				...
				// start consumer when (spend > consumerMaxIntervalTimeMillis|| container size
				// more than consumerMaxCount) && container has value
				if ((spend >= consumerMaxIntervalTimeMillis || container.size() >= consumerMaxCount)
						&& container.size() > 0) {
